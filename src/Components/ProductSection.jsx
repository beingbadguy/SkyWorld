    ...
    <div className=" px-4 md:px-12 py-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-center">
        {data.map((p, index) => (
          <div
            key={index}
            className="relative flex items-center justify-center flex-col gap-4 bg-gray-100 p-7 rounded cursor-pointer"
          >
            <div
              className="w-36 h-36 sm:h-48 sm:w-48 md:h-54 md:w-54  flex items-center justify-center rounded-full  overflow-hidden"
              onClick={() => {
            ...
            >
              <p className="md:text-xl  text-gray-700">{p.name}</p>
              {/* <div>
                {Array.from({ length: p.rating }, (_, index) => (
                  <IoStarSharp key={index} className="text-pink-400" />
                ))}
              </div> */}
